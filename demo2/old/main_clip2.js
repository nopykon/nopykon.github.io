...
//let inn_push = a => qcpy(inn[inn_sz++],a);
//let out_push = a => qcpy(out[out_sz++],a);
function inn_push(a){return qcpy(inn[inn_sz++],a);}
function out_push(a){return qcpy(out[out_sz++],a);}
...
function clip_n_paint( vv, vc ) //[x,y,u,v,L]
{
	...
	if( inn.length > 2 ){
		...
		ct.beginPath();
		ct.moveTo(inn[0][0],inn[0][1]);
		for(let i=1;i<inn.length;++i)
			ct.lineTo(inn[i][0],inn[i][1]);
		ct.closePath();
		ct.stroke();
	}
	//draw out
	if( out.length > 2 ){
		ct.strokeStyle ='blue';
		ct.beginPath();
		ct.moveTo(out[0][0],out[0][1]);
		for(let i=1;i<out.length;++i)
			ct.lineTo(out[i][0],out[i][1]);
		ct.closePath();
		ct.stroke();
	}
	*/
